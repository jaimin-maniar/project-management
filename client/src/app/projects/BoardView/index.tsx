...
import { EllipsisVertical, MessageSquareMore, PlusCircle } from "lucide-react";
    ...
    >
      ...
      <div className="p-4 md:p-6">
        <div className="flex items-start justify-between">
          <div className="fkex-1 items-centr flex flex-wrap gap-2">
            {task.priority && <PriorityTag priority={task.priority} />}
            <div className="flex gap-2">
              {taskTagsSplit.map((tag) => (
                <div
                  key={tag}
                  className="rounded-full bg-blue-100 px-2 py-1 text-xs font-semibold dark:text-black"
                >
                  {" "}
                  {tag}
                </div>
        ...
        <div className="text-xs text-gray-500 dark:text-neutral-500">
          ...
          {formattedDueDate && <span>{formattedStartDate}</span>}
        </div>
